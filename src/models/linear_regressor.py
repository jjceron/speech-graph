...
def _metric_pattern(metric: str) -> str:
    return rf"(?:^a\d+_{metric}$|^w\d+_a\d+_(?:mean_|std_|global_)?{metric}$|^{metric}$)"


def _is_model_feature(col: str, feature_set: str) -> bool:
    if feature_set == "canonical":
        return col in MODEL_METRICS
    if feature_set == "by_activity":
        return any(re.fullmatch(rf"a\d+_{metric}", col) for metric in MODEL_METRICS)
    if feature_set == "activity_window_wide":
        return any(re.fullmatch(rf"w\d+_a\d+_{metric}", col) for metric in MODEL_METRICS)
    if feature_set == "full":
        return any(re.fullmatch(rf"w\d+_a\d+_(?:mean_|std_|global_)?{metric}", col) for metric in MODEL_METRICS)
    return any(re.fullmatch(_metric_pattern(metric), col) for metric in MODEL_METRICS)


def select_features(df: pd.DataFrame, targets: dict[str, str], feature_set: str = "auto") -> list[str]:
...
def _pipeline(model_type: str, alpha: float, alphas: list[float] | None) -> Pipeline:
    ...
    elif model_type == "ridgecv" or alphas:
        estimator = RidgeCV(alphas=np.asarray(alphas or DEFAULT_ALPHAS, dtype=float))
    else:
...
def cross_validated_model(
    ...
    for fold, (train_idx, test_idx) in enumerate(split_iter, start=1):
        ...
        for feature, coefficient in zip(feature_cols, coefficients):
            coef_rows.append(context | {"target": target_label, "fold": fold, "feature": feature, "coefficient": float(coefficient), "alpha": chosen_alpha})
        row_meta = work.iloc[test_idx].reset_index(drop=True).copy()
        for key, value in context.items():
            row_meta[key] = value
        contrib_frames.append(linear_contributions(x_test_scaled, coefficients, feature_cols, row_meta, target_label, fold))

    meta_cols = [col for col in ["code", "Cod", "Age", "Edad", "School year", "Educational level", "Escolaridad", "Gender", "Tipo", "activity", "activity_number", "window_size", "scheme_window_size"] if col in work.columns]
...
def _activity_window_groups(df: pd.DataFrame):
...
def _run_one_matrix(
    ...
    grouped = _activity_window_groups(df) if by_activity_window else []
    if grouped:
        for (_, _), sub in grouped:
            context = _context_from_df(sub)
            feature_cols = select_features(sub, target_map, feature_set="canonical")
            max_features = max(max_features, len(feature_cols))
            if not feature_cols:
                continue
            for target_label, target_col in target_map.items():
                result, predictions, coefficients, contributions = cross_validated_model(
                    sub, target_label, target_col, feature_cols, group_col, n_splits, model_type, alpha, alphas, context=context
                )
    ...
    return (
        ...
        pd.concat(contrib_frames, ignore_index=True) if contrib_frames else pd.DataFrame(),
        max_features,
    )
...
def run_linear_regression(
    ...
    results_df, predictions_df, coefficients_df, contributions_df, max_features = _run_one_matrix(
        df=df,
        ...
        alpha=alpha,
        alphas=alphas,
        by_activity_window=by_activity_window,
    )
    ...
    mode = "activity_window" if by_activity_window and {"activity_number", "window_size"}.issubset(df.columns) else feature_set
    print(f"Model mode: {mode}. Max features per model: {max_features}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--feature-set", default="auto", choices=["canonical", "by_activity", "activity_window_wide", "full", "auto"])
    parser.add_argument("--model", default="ridge", choices=["ridge", "ridgecv", "linear"])
    parser.add_argument("--alpha", type=float, default=1.0)
    parser.add_argument("--alphas", default="", help="Optional comma-separated alphas for RidgeCV inside each fold.")
    parser.add_argument("--by-activity-window", dest="by_activity_window", action="store_true", default=True)
    parser.add_argument("--pooled", dest="by_activity_window", action="store_false")
...
def main() -> None:
    ...
    run_linear_regression(
        ...
        by_activity_window=args.by_activity_window,
    )
