...
GRAPH_LABEL_PLACEHOLDER = "zzzgraphlabelzzz"
...
def clean_text_for_nlp(text: str, lowercase: bool = True, normalize_annotations: bool = True) -> str:
    ...
    value = value.replace("[[EE]]", f" {GRAPH_LABEL_PLACEHOLDER} ")
    ...
    value = value.replace(GRAPH_LABEL_PLACEHOLDER.lower(), "[[EE]]")
    value = value.replace(GRAPH_LABEL_PLACEHOLDER, "[[EE]]")
    value = value.replace(BREAK_PLACEHOLDER.lower(), BREAK_TOKEN)
    value = value.replace(BREAK_PLACEHOLDER, BREAK_TOKEN)
    return value
...
def tokenize_segments(text: str, lowercase: bool = True, normalize_annotations: bool = True) -> list[list[str]]:
    ...
    for part in cleaned.split(BREAK_TOKEN):
        tokens = [match.group(0) for match in WORD_RE.finditer(part)]
        if tokens:
