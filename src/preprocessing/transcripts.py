...
def parse_transinfo(lines: list[str]) -> dict[str, str]:
    for line in lines:
        ...
        if not match:
            continue
        return {key: value.strip() for key, value in TRANSINFO_ATTR_RE.findall(match.group(1))}
    return {}
...
def parse_transcript(path: Path, include_speakers: Iterable[str] = ("spk_1",)) -> Transcript:
    ...
    include_set = {speaker.strip().lower() for speaker in include_speakers}
    ...
    for raw_line in lines:
        ...
        if activity_match:
            ...
            activity = canonical_activity(activity_match.group(1).strip())
            current = Activity(name=activity.canonical, start_time=activity_match.group(2), end_time=activity_match.group(3))
...
def iter_transcripts(directory: Path, include_speakers: Iterable[str] = ("spk_1",), max_files: int | None = None) -> list[Transcript]:
    ...
    for index, path in enumerate(sorted(Path(directory).glob("*.txt"))):
